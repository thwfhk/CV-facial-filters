...
def main(args):
    ...
    for img_fp in args.files:
        ...
        if args.dlib_bbox:
            rects = face_detector(img_ori, 1) # dlib.rectangle
        else:
            rects = []
        ...
        for rect in rects:
            ...
            input = transform(img).unsqueeze(0) #transform!!!
            with torch.no_grad():
                if args.mode == 'gpu':
                    input = input.cuda()
                param = model(input)
        ...
        if args.dump_pncc:
            ...
            print('Dump to {}'.format(wfp))
        if args.dump_res:
            draw_landmarks(img_ori, pts_res, style="simple", wfp=img_fp.replace(suffix, '_3DDFA.jpg'), show_flg=args.show_flg)
