...
class twh:
    ...
    def get_landmarks(self, img):  # in rgb
        floc = self.detector.detect(img[:, :, ::-1])
        face_locations = []
        faces_landmarks = []
        faceRects = []
        ...
        # NOTE: using 3ddfa
        faces_landmarks, Ps, poses, pts_3ds, roi_boxes = self.landmarkor.meow_landmarks(img, faceRects, use_landmarks=False)

        return face_locations, faces_landmarks, Ps, poses, pts_3ds, roi_boxes


    def plot(self, img, face_locations, faces_landmarks, Ps, poses, pts_3ds, roi_boxes, selected_filters):
        '''
